...
class SettingsUI(QWidget):
    ...
    def _build_integrations(self):
        w = QWidget(); root = QVBoxLayout(w)
        box, layout = self._section("Spotify", "Connect Spotify and configure its access without leaving Settings.")
    ...
    def _build_permissions(self):
        w=QWidget(); root=QVBoxLayout(w)
        box,layout=self._section("Tool Access","Set access for individual capabilities. Allow runs automatically, Confirm asks before execution, Deny blocks it.")
        ...
        root.addWidget(box)
        box,layout=self._section("Automation","The existing automation safety mode remains available as a global control.")
